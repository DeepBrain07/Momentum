...
const ArticleCard: React.FC<ArticleCardProps> = ({title, category, image, detailsLink, date}) => {
    ...
    return (
        <div onClick={()=>navigate(detailsLink)} className="w-[100%] shadow-md flex flex-col border rounded-md max-w-[280px] p-4 min-h-[240px] items-center justify-center gap-4">
            <img src={image} alt="servide image" className="rounded-md w-[100%] h-[130px]"/>
            <div className="w-[100%] flex flex-col gap-4">
                <div className="flex justify-between">
                    <p className={`text-[#000] text-[14px] font-bold tracking-wider`}>{category}</p>
